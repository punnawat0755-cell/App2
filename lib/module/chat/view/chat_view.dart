...
class _WaitingChatPageState extends State<WaitingChatPage>
  ...
  Future<void> _attemptMatchCycle() async {
    ...
    try {
      ...
      .set({
        'status': 'waiting',
        'role': widget.role == MatchRole.listener ? 'listener' : 'seeker',
        'sb_id': supabase.auth.currentUser?.id,
        'updatedAt': FieldValue.serverTimestamp(),
      }, SetOptions(merge: true)
      );

      await FirebaseFirestore.instance.collection('chat').doc(chatId).set({
        'users': [userA_FirebaseId, userB_FirebaseId],
        'randomState': 'Active',
        'createdAt': FieldValue.serverTimestamp(),
        'seeker_sb_id': seekerSbId,
        'listener_sb_id': listenerSbId,
      });
...
class _ChatPageState extends State<ChatPage> {
  ...
  void _watchChatEndedByPeer() {
    ...
    void _exitByPeerEnd() {
      ...
      // เปิดหน้า feedback ให้ฝั่งที่ถูกจบด้วย
      if (mounted) {
        setState(() {
          _showFeedback = true;
        });
 ...
 Future<void> _submitFeedback() async {
  ...
  try {
    ...
    // ปิดหน้า feedback (เพราะมันเป็น overlay ในหน้าเดียวกัน)
    setState(() => _showFeedback = false);

    // ถ้าคุณอยาก "ออกจากหน้าแชท" หลังส่งฟีดแบค ให้เปิดบรรทัดนี้
    // Get.back();
  } catch (e) {
  ...
  @override
  Widget build(BuildContext context) {
    // ถ้าต้องการให้ตอน _showFeedback เป็น true แล้วแสดงหน้าให้คะแนนทันที
    if (_showFeedback) {
