...
class _FeedPageState extends State<FeedPage> {
  ...
  Future<void> _deletePost(FeedPost post) async {
    ...
    try {
      ...
      if (!mounted) return;
      _showSnackBar('ลบโพสต์เรียบร้อยแล้ว');
    } on FeedPostUnavailableException {
      if (!mounted) return;
      _showSnackBar('ไม่พบโพสต์นี้แล้ว หรือคุณไม่มีสิทธิ์ลบ', isError: true);
    } catch (error) {
      if (!mounted) return;
...
class _FeedProfilePageState extends State<FeedProfilePage> {
  ...
  Future<void> _deletePost(BuildContext context, FeedPost post) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('ลบโพสต์นี้ออกจากโปรไฟล์?'),
        content: const Text(
          'โพสต์นี้จะถูกลบออกจากโปรไฟล์และ feed ทันที ไม่สามารถกู้คืนได้',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(false),
    ...
    try {
      ...
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('ลบโพสต์จากโปรไฟล์เรียบร้อยแล้ว')),
      );
    } on FeedPostUnavailableException {
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('ไม่พบโพสต์นี้แล้ว หรือคุณไม่มีสิทธิ์ลบ'),
          backgroundColor: Colors.red,
        ),
      );
    } catch (error) {
                    ...
                    return ListView(
                      children: [
                        ...
                        SizedBox(
                          ...
                          child: _FeedMessageState(
                            icon: Icons.cloud_off_rounded,
                            title: 'โหลดโพสต์ในโปรไฟล์ไม่สำเร็จ',
                            subtitle: '${postSnapshot.error}',
                          ),
                        ),
                      ],
                            ...
                            if (posts.isEmpty) {
                              return SizedBox(
                                ...
                                child: _FeedMessageState(
                                  ...
                                  title: 'ยังไม่มีโพสต์ในโปรไฟล์นี้',
                                  subtitle:
                                      'เมื่อเจ้าของโปรไฟล์สร้างโพสต์ เนื้อหาจะแสดงที่นี่ทันที',
                                ),
                              );
                            }
...
class _FeedComposerSheetState extends State<_FeedComposerSheet> {
  ...
  String _messageForModerationReason(String reason) {
    ...
    return 'ไม่สามารถโพสต์ข้อความนี้ได้ เนื่องจากผลการตรวจสอบไม่ผ่าน';
  }
